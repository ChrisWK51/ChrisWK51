...
For future upgrades, set the Preview version and review a branch deployment before merging into `master`. Coordinate the Production version change with the merge so both environments use the version pinned in `.hugo-version`. For the current site, confirm the log reports `0.166.0` and `+extended`.
...
Branding instructions are in [branding.md](branding.md). The repository's [main README](../README.md) is displayed on the GitHub profile.
